...
const zhCN = {
  ...
  backend: {
    ...
    probe: "探测能力",
    probing: "探测中",
    probeSuccess: "探测完成，发现 {{count}} 项能力",
    probeSuspicious: "探测结果不可信——所有端点都返回 200，这个后端可能有 catch-all 兜底。请手动勾选实际支持的能力。",
    probeFailed: "探测失败，检查地址是否正确、后端是否允许跨域",
    capabilities: "支持的能力",
    capabilityNames: { chat: "聊天", image: "绘图", video: "视频", tts: "语音合成", stt: "语音识别" },
    flavor: "后端类型",
    lastProbed: "上次探测：{{time}}",
    neverProbed: "尚未探测",
    models: "模型",
...
const en = {
  ...
  backend: {
    ...
    probing: "Probing",
    probeSuccess: "Found {{count}} capabilities",
    probeSuspicious: "Probe result is unreliable — every endpoint returned 200, so this backend likely has a catch-all fallback. Please select the supported capabilities manually.",
    probeFailed: "Probe failed. Check the URL and whether the backend allows cross-origin requests.",
    capabilities: "Capabilities",
    capabilityNames: { chat: "Chat", image: "Image", video: "Video", tts: "Speech", stt: "Transcribe" },
    flavor: "Backend type",
    lastProbed: "Last probed: {{time}}",
