...
Cloudflare 部署地址启用 HTTPS 后，Android Chrome 会在地址栏或菜单提供「安装应用」；应用侧栏底部也会显示安装按钮。iPhone/iPad 用 Safari 打开后，从分享菜单选择「添加到主屏幕」。安装后会以独立窗口启动，聊天数据和后端配置仍保存在本机浏览器中。
...
勾选是攒着的，改完点保存才落盘 —— 一勾一存的话列表会跟着刷新、滚动位置丢，68 行里挑十几个体感很差。归类、图片路由和视频路由的改动也一起攒在这个草稿里，免得同一个页面有的控件即时生效、有的要点保存。草稿状态挂在设置页外面，中途切去别的面板再回来不会白勾。设置页的排序只按提供商和 id，勾选与否不影响位置 —— 那一页是一边扫一边勾的，列表不该在手底下动。
...
设置页有独立的「语音」标签：聊天麦克风默认隐藏，可按需打开。聊天输入框采用微信式交互：点麦克风切到语音输入，按住整块输入区域，松开后结束并转写；语音页的录音入口也固定为按住说话。
...
图片路由暂不支持异步任务轮询 —— 目前接触到的图片端点都是同步返回的。视频那边有轮询，见下。

## 视频路由

视频遇到的是和图片一模一样的问题：有的提供商用 OpenAI 的任务接口（`/videos/generations` 提交、`/videos/{id}` 轮询），有的把视频生成直接挂在 `/chat/completions` 上同步返回，视频地址写在回复正文里。这同样从模型 id 上看不出来，所以用同一套路由机制，模板语法跟图片路由完全一致。

内置两条：`videos`（任务接口，默认）和 `chat`（对话端点）。**用对话端点的提供商，在「模型」页把那个模型的路由改成「对话端点 /chat/completions」就行，不用自己写 JSON。**
...
- 可用变量 `model prompt messageContent duration aspectRatio resolution sourceUrl`，下划线写法同样认。图生视频用 `sourceUrl`（本地文件会先经 Worker 传成公网 URL）或 `messageContent`（OpenAI 多模态消息）。模板里没引用源地址的路由，面板不显示附件按钮。
- **`statusPath` 决定这条路由是异步还是同步**：填了就用它轮询任务状态（`${requestId}` 替换成提交时拿到的任务 ID，会做 URL 编码），留空表示同步 —— 提交响应里就该有视频地址，没有直接报错，不去轮询一个不存在的端点。
- `videoUrlPaths` 留空就用通用提取：先深挖 `video_url` / `output_url` / `video.url` 一类字段，再扫回复正文里的链接。**正文里带 `.mp4`/`.webm` 一类扩展名的链接最可信，没有扩展名时退回第一条普通 markdown 链接**（签名 URL 常常不带扩展名）；`![](…)` 图片语法当作封面图跳过。认不出来的形状再用 `videoUrlPaths` 显式指定。
- 「编辑」「延长」只有内置 `videos` 路由支持 —— 它们打的是 `/videos/edits` 和 `/videos/extensions`，语义由那套任务接口定死，对话端点没有对应概念。选其它路由时面板会把这两个操作收起来。
