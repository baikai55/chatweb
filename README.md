...
模型列表本身缓存在 IndexedDB，24 小时内不重复请求（一次拉取实际是三个请求：`/v1/models` 加两个富字段端点）。设置页有手动刷新。
...
- **归类对了不等于端点能用**：Nano Banana 系列确实是图片模型，但 CPA 不允许它们走 `/images/generations`（上游报错会列出该部署实际支持的模型，面板原样展示）。这类"模型存在但此端点不接"的情况靠分类规则解决不了，只能把上游错误如实透出来
