...
- **对话**：OpenAI Chat Completions、图片输入、流式输出、推理过程、工具活动、停止生成和会话历史。
...
- **浏览器里的「联网」和 opencode 那种联网不是一回事**：这里发的是 `tools:[{type:"web_search"}]`，请上游用它自己的内置搜索；opencode 是把搜索做成一个由客户端执行的 function tool，模型只管发调用、真正去搜的是客户端。所以上游没有内置搜索时 opencode 那条路仍然能用 —— 要在这里做到同样效果，得实现工具调用循环外加一个搜索源，还没做
