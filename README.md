...
四个面板都有历史记录，刷新不丢。侧边栏显示哪几个面板由设置页手动勾选。
...
每个后端每个面板留最近 50 条，超了删最旧的。设置页的「行为」页底部有「删除全部记录」，清掉所有后端的对话和生成记录，但不动后端配置、密钥和模型缓存 —— 那些删了得重新填一遍。
...
- **语音端点只有 grok2api 有**：CPA 的 `/tts`、`/stt`、`/audio/speech`、`/audio/transcriptions` 全是 404，尽管它的模型表里有 8 个 TTS 模型。语音面板因此还要看 `flavor`，不能只看模型分类
- **联网搜索按钮不支持时禁用而不是隐藏**：早先是隐藏的，结果模型 id 里没写 gemini / grok 时按钮凭空消失，看起来像功能丢了。现在一直在，禁用状态的 tooltip 说明原因（Claude 得走 `/v1/messages`，还没做）
